...
const DUMMY_SERMONS = [
  {
    ...
    image_url: "https://images.unsplash.com/photo-1438232992925-8e3b97b0036a?q=80&w=2000&auto=format&fit=crop",
  ...
  },
  {
    ...
    image_url: "https://images.unsplash.com/photo-1490730141103-6cac27aaab94?q=80&w=2000&auto=format&fit=crop",
    video_url: "https://www.youtube.com/watch?v=dQw4w9WgXcQ"
  }
];
  ...
  const loadSermons = async () => {
    try {
      ...
      if (recentData && recentData.length > 0) {
        ...
        setSermons(DUMMY_SERMONS.slice(0, 5));
      }

      if (allData && allData.length > 0) {
        setAllSermons(allData);
      } else {
        setAllSermons(DUMMY_SERMONS);
      }
    } catch (err) {
      console.error("Failed to load sermons, using dummy data:", err);
      setSermons(DUMMY_SERMONS.slice(0, 5));
      setAllSermons(DUMMY_SERMONS);
    } finally {
      setLoading(false);
            ...
            {filteredSermons.length > 0 ? (
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                {filteredSermons.map((sermon, index) => (
                  <div 
                    key={index} 
                    className="group bg-white/5 rounded-2xl overflow-hidden border border-white/10 hover:border-primary/50 transition-all animate-in zoom-in-95 duration-500 delay-[index*50ms]"
                  >
